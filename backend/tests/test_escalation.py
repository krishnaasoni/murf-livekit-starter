...
async def test_create_escalation_record_db(tmp_path):
    """Test creating and retrieving human escalation record in SQLite DB with custom ticket_code."""
    ...
    assert "ticket_code" in res
    assert res["ticket_code"].startswith("ECW_")

    records = db.get_escalations(db_path=test_db)
    assert len(records) == 1
    assert records[0]["caller_name"] == "Ramesh Kumar"
    assert records[0]["ticket_code"] == res["ticket_code"]

    # Test resolving escalation by ticket_code
    resolved = db.resolve_escalation(res["ticket_code"], db_path=test_db)
...
@pytest.mark.asyncio
async def test_call_analytics_recording(tmp_path):
    """Test recording and retrieving call analytics (SUCCESS and FAILED outcomes)."""
    test_db = str(tmp_path / "test_analytics.db")
    db.init_db(db_path=test_db)

    db.record_call_analytics(caller_id="Krishna", outcome="SUCCESS", db_path=test_db)
    db.record_call_analytics(
        caller_id="Anonymous", outcome="FAILED", db_path=test_db
    )
    ...
    assert summary["recent_calls"][0]["outcome"] == "FAILED"
    assert summary["recent_calls"][1]["outcome"] == "SUCCESS"
